...
$stmtStats = $pdo->query("
    SELECT 
        (SELECT COUNT(*) FROM Noticia) as total_noticias,
        (SELECT COUNT(*) FROM Noticia WHERE status = 'publicada') as publicadas,
        (SELECT COUNT(*) FROM Noticia WHERE status = 'pendente') as pendentes,
        (SELECT COUNT(*) FROM Escritor) as escritores,
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
                <h1 class="text-white fw-bold mb-1">
                    <i class="bi bi-person-badge text-primary me-3"></i>Meu Perfil
                </h1>
                <p class="text-secondary mb-0">Gerencie suas informações de administrador</p>
            </div>
                ...
                <div class="card border-0 rounded-4 shadow-lg text-center" style="background: rgba(30, 41, 59, 0.9);">
                    <div class="card-body p-5">
                        ...
                        <div class="rounded-circle mx-auto mb-4 d-flex align-items-center justify-content-center" 
                             style="width: 120px; height: 120px; background: linear-gradient(135deg, #EF4444, #F97316);">
                            <i class="bi bi-shield-fill-check text-white" style="font-size: 4rem;"></i>
                        </div>
                        ...
                        <!-- Estatísticas do Sistema -->
                        <h6 class="text-secondary mb-3">Estatísticas do Sistema</h6>
                        <div class="row g-2 text-center">
                            <div class="col-6">
                                <div class="bg-dark rounded-3 p-2">
                                    <h5 class="text-primary mb-0"><?= $estatisticas['total_noticias'] ?? 0 ?></h5>
                                    <small class="text-secondary" style="font-size: 0.75rem;">Notícias</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-dark rounded-3 p-2">
                                    <h5 class="text-warning mb-0"><?= $estatisticas['pendentes'] ?? 0 ?></h5>
                                    <small class="text-secondary" style="font-size: 0.75rem;">Pendentes</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-dark rounded-3 p-2">
                                    <h5 class="text-success mb-0"><?= $estatisticas['escritores'] ?? 0 ?></h5>
                                    <small class="text-secondary" style="font-size: 0.75rem;">Escritores</small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="bg-dark rounded-3 p-2">
                                    <h5 class="text-danger mb-0"><?= $estatisticas['admins'] ?? 0 ?></h5>
                                    <small class="text-secondary" style="font-size: 0.75rem;">Admins</small>
                                </div>
                            </div>
                        </div>
                    </div>
